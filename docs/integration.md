...
## Четыре способа отправить обращение

### Страница `/form` — окно в браузере

Самый короткий путь для всего, что не веб-игра: Unity, Electron, нативный
десктоп, лаунчер. Шлюз держит страницу с той же формой, что у JS SDK; игра
открывает её в системном браузере с токеном во фрагменте адреса:

```
https://support.flushee.work/form#token=<jwt>
```

Фрагмент в запрос не уходит — токен не попадает ни на сервер, ни в его логи,
а страница сразу стирает его из адресной строки. Дальше всё как в веб-игре:
категории, файлы через диалог браузера, экран «отправлено» с номером
обращения; сессия продлевается через шлюз до 3 часов, а без токена или после
предельного срока страница просит открыть её из игры заново. Токен для этого
пути выпускается так же, как для всех остальных.

Чего здесь нет: итога отправки в игре (браузер — отдельный процесс) и
скриншота игры — игрок прикладывает свой. Когда окно должно быть в самой
игре — способы ниже.
...
- **Форма в браузере** — `SupportGateBrowser.Open(endpoint, token)` открывает
  страницу `/form` (выше) в системном браузере: без плагинов и WebView,
  работает на iOS, Android, десктопе и в WebGL.
